  ...
  return (
    <Drawer.Root 
      ...
      onOpenChange={(open) => {
        if (!open) {
          onClose();
          setTimeout(() => setShowCal(false), 300); // reset state after closing animation
        }
      }}
    >
      <Drawer.Portal>
        <Drawer.Overlay className="fixed inset-0 z-50 bg-black/60 backdrop-blur-sm" />
        <Drawer.Content className="fixed bottom-0 left-0 right-0 z-50 mt-24 flex flex-col rounded-t-xl bg-[var(--bg-elevated)] border-t border-l border-r border-[var(--border-subtle)] max-h-[96vh] focus:outline-none">
          <div className="mx-auto mt-4 h-1.5 w-12 flex-shrink-0 rounded-full bg-[var(--border-subtle)]" />
          
          <Drawer.Title className="sr-only">Contact Form</Drawer.Title>
          <Drawer.Description className="sr-only">Choose a way to contact Felipe</Drawer.Description>
          
          {/* Cabeçalho */}
          <header className="flex items-center justify-between px-6 py-4 border-b border-[var(--border-subtle)] mt-2">
          ...
          {/* Corpo */}
          <div className="p-6 md:p-8 pb-32 flex flex-col gap-8 overflow-y-auto min-h-[50vh]">
