...
import { PlusCircle, Trash2, ChevronDown, AlertTriangle, Scale, Layers, Calculator, Check, ArrowLeft } from 'lucide-react';
...
const FABRIC_COMPOSITIONS = [
  '100% ALGODÃO', '100% POLIÉSTER', '50% ALGODÃO 50% POLI', 'VISCOSE', 'MALHA PV', 'OUTRO',
];
...
interface ColorVariantDraft {
  id: string; color: string; weightKg: string;
...
interface FabricDraft {
  id: string; fabricSupplier: string; fabricComposition: string;
  customFabricComposition: string; fabricType: string;
  totalWeightKg: string; yieldRatio: string; variants: ColorVariantDraft[];
}
interface ActiveCell { fabricId: string; variantId: string; size: SizeKey | '__weightKg'; label: string; }
...
function emptyFabric(id?: string): FabricDraft {
  return { id: id || uid(), fabricSupplier: '', fabricComposition: FABRIC_COMPOSITIONS[0], customFabricComposition: '', fabricType: '', totalWeightKg: '', yieldRatio: '', variants: [emptyVariant()] };
}
...
export default function EditOpPage() {
  ...
  useEffect(() => {
    ...
    getProductionOrderById(id).then(op => {
      if (op) {
        setOpNumber(op.opNumber);
        setModelReference(op.modelReference);
        setClientName(op.clientName);
        if (MODEL_OPTIONS.find(o => o.value === op.modelType)) {
          setModelType(op.modelType);
        } else {
          setModelType('OUTRO'); setCustomModelType(op.modelType);
        }
        setModelDescription(op.modelDescription || '');
        setPriority(op.priority || 'NORMAL');
        if (op.timestamps?.deadline) {
          try {
            const date = new Date((op.timestamps.deadline as any).seconds * 1000);
            setDeadlineDate(date.toISOString().split('T')[0]);
          } catch(e) {}
        }
        setNotes(op.notes || '');

        const initialFabrics = op.fabrics && op.fabrics.length > 0 ? op.fabrics : [{
          id: 'FAB_LEGACY', fabricSupplier: op.fabricSupplier || '', fabricComposition: op.fabricComposition || '',
          fabricType: op.fabricType || '', totalFabricWeightKg: op.totalFabricWeightKg || 0,
          nominalYieldRatio: op.nominalYieldRatio || 0, matrixGrid: op.matrixGrid || []
        }];

        setFabrics(initialFabrics.map(f => ({
          id: f.id || `FAB_${uid()}`,
          fabricSupplier: f.fabricSupplier,
          fabricComposition: FABRIC_COMPOSITIONS.includes(f.fabricComposition) ? f.fabricComposition : 'OUTRO',
          customFabricComposition: !FABRIC_COMPOSITIONS.includes(f.fabricComposition) ? f.fabricComposition : '',
          fabricType: f.fabricType,
          totalWeightKg: String(f.totalFabricWeightKg),
          yieldRatio: String(f.nominalYieldRatio),
          variants: f.matrixGrid.map((v: any) => ({
            id: `VAR_${uid()}`,
            color: v.color,
            weightKg: String(v.weightKg),
            sizes: Object.fromEntries(Object.entries(v.sizes).map(([s, d]: [string, any]) => [s, { estimated: d.estimated }]))
          }))
        })));
      }
  ...
  const { totalFabricWeightKg, totalEstimatedPieces } = useMemo(() => {
    ...
    for (const f of fabrics) {
      const fw = parseFloat(f.totalWeightKg) || 0; weight += fw;
  ...
  const openNumpad = useCallback((cell: ActiveCell, initialValue: string) => {
    activeCellRef.current = cell; setActiveCell(cell); setNumpadValue(initialValue);
  }, []);
  const closeNumpad = useCallback(() => { activeCellRef.current = null; setActiveCell(null); setNumpadValue(''); }, []);
  ...
  const toggleSize = useCallback((fabricId: string, variantId: string, size: SizeKey) => {
    setFabrics(prev => prev.map(f => {
      ...
      return { ...f, variants: f.variants.map(v => {
        ...
        const sizes = { ...v.sizes };
        if (sizes[size]) { delete sizes[size]; setActiveCell(curr => curr?.fabricId === fabricId && curr.variantId === variantId && curr.size === size ? null : curr); }
        else sizes[size] = { estimated: 0 };
        return { ...v, sizes };
      }) };
    }));
  }, []);
  ...
  const applyProportions = useCallback((fabricId: string, variantId: string) => {
    setFabrics(prev => prev.map(f => {
      ...
      return { ...f, variants: f.variants.map(v => {
        ...
        for (const size of activeSizes) { if (propValues[size] > 0) newSizes[size] = { estimated: Math.round(quota * propValues[size]) }; }
  ...
  const handleSubmit = async () => {
    ...
    try {
      const finalFabrics: FabricEntry[] = fabrics.map(f => {
        const matrixGrid: MatrixGridVariant[] = f.variants.map(v => {
          ...
          for (const size of ALL_SIZES) {
            ...
            if (entry) { sizesRecord[size] = { estimated: entry.estimated, realCut: 0, readySewn: 0, approvedQc: 0 }; totalEst += entry.estimated; }
          }
      ...
      await updateProductionOrder(id, {
        ...
        priority,
        timestamps: { createdAt: Timestamp.now(), deadline: Timestamp.fromDate(new Date(deadlineDate + 'T12:00:00Z')) },
        notes: notes.trim() || undefined,
      });
      router.push(`/pt/sandbox/dora-mes/ordens/${id}`);
    } catch (err) {
      console.error(err);
  ...
  return (
    <>
      <main className="pb-44 p-4 space-y-5 max-w-2xl mx-auto">
        ...
        {/* ── Header ── */}
        <header className="pb-4 flex items-center gap-3" style={{ borderBottom: '1px solid rgba(255,255,255,0.08)' }}>
          <button
            onClick={() => router.back()}
            className="h-10 w-10 rounded-xl flex items-center justify-center shrink-0 active:scale-[0.97] transition-all"
            style={{ background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.1)' }}
          >
            <ArrowLeft size={20} className="text-text-primary" />
          </button>
          <div className="flex-1 min-w-0">
            <h1 className="font-black text-xl tracking-tight text-text-primary">Editar Ordem de Produção</h1>
            <p className="text-xs text-text-secondary">Atualize os dados e a grade de tamanhos</p>
          </div>
        ...
        <section className="space-y-4 rounded-2xl p-4" style={cardStyle}>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <input type="text" value={modelReference} onChange={e => setModelReference(e.target.value)} placeholder="Ex: F2401" className={`${inputMonoClass} uppercase`} />
            </div>
            <div>
              <label className={labelClass}>Descrição do Modelo</label>
              <input type="text" value={modelDescription} onChange={e => setModelDescription(e.target.value)} placeholder="Ex: Camisa M.Malha Alg." className={inputClass} />
            </div>
            <div className="col-span-2">
              <label className={labelClass}>Cliente</label>
              <input type="text" value={clientName} onChange={e => setClientName(e.target.value)} placeholder="Ex: Frank Arts Estamparia" className={inputClass} />
            </div>
            <div className="col-span-2">
              ...
              {modelType === 'OUTRO' && (
                <input type="text" value={customModelType} onChange={e => setCustomModelType(e.target.value)} placeholder="Especifique o modelo..." className={`${inputClass} mt-2 uppercase`} />
              )}
            </div>
            <div>
            ...
            <div>
              ...
              <input type="date" value={deadlineDate} onChange={e => setDeadlineDate(e.target.value)}
                     min={new Date().toISOString().split('T')[0]} className={inputClass} style={{ colorScheme: 'dark' }} />
            </div>
          </div>
        </section>
        ...
        {fabrics.map((fabric, fIndex) => {
          ...
          return (
            <div key={fabric.id} className="space-y-4 rounded-2xl p-4" style={cardStyle}>
              <div className="flex items-center justify-between">
                <h2 className="text-[10px] font-black text-text-muted uppercase tracking-widest flex items-center gap-2">
                  ...
                  {fabricEstimatedPieces && (
                    <span className="font-mono font-black text-factory-sew text-xs">· {fabricEstimatedPieces.toLocaleString('pt-BR')} pçs</span>
                  )}
                </h2>
                {fabrics.length > 1 && (
              ...
              <div className="grid grid-cols-2 gap-3 rounded-xl p-3" style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.06)' }}>
                <div className="col-span-2">
                  <label className={labelClass}>Fornecedor de Tecido</label>
                  <input type="text" value={fabric.fabricSupplier} onChange={e => updateFabric(fabric.id, 'fabricSupplier', e.target.value)} placeholder="Ex: Quatro K Textil Ltda" className={inputClass} />
                </div>
                <div className="col-span-2 sm:col-span-1">
                  ...
                  {fabric.fabricComposition === 'OUTRO' && (
                    <input type="text" value={fabric.customFabricComposition} onChange={e => updateFabric(fabric.id, 'customFabricComposition', e.target.value)} placeholder="Ex: 96% VISCOSE 4% ELASTANO" className={`${inputClass} mt-2 uppercase text-sm`} />
                  )}
                </div>
                <div className="col-span-2 sm:col-span-1">
                  <label className={labelClass}>Tipo de Malha</label>
                  <input type="text" value={fabric.fabricType} onChange={e => updateFabric(fabric.id, 'fabricType', e.target.value)} placeholder="Ex: MM Pent. Esp." className={inputClass} />
                </div>
                <div>
              ...
              {fabric.variants.map(variant => {
                ...
                return (
                  <div key={variant.id} className="rounded-xl overflow-hidden" style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.08)' }}>
                    <div className="flex items-center gap-2 p-3" style={{ background: 'rgba(255,255,255,0.05)', borderBottom: '1px solid rgba(255,255,255,0.07)' }}>
                      <input type="text" value={variant.color} onChange={e => updateVariantColor(fabric.id, variant.id, e.target.value)}
                             placeholder="COR (Ex: PRETO)" className="flex-1 bg-transparent font-black text-text-primary placeholder-text-muted focus:outline-none uppercase text-sm tracking-widest" />
                      <button
                        onPointerDown={() => openNumpad({ fabricId: fabric.id, variantId: variant.id, size: '__weightKg', label: `Peso — ${variant.color || 'Cor'}` }, variant.weightKg)}
                      ...
                      >
                        <Scale size={12} />{variant.weightKg ? `${variant.weightKg}kg` : '? kg'}
                      </button>
                      {fabric.variants.length > 1 && (
                        <button onPointerDown={() => removeVariant(fabric.id, variant.id)} className="p-1.5 text-text-muted hover:text-factory-scrap rounded-lg transition-colors">
                          <Trash2 size={16} />
                        </button>
                      )}
                    </div>

                    <div className="p-3 pb-2 flex items-center justify-between" style={{ borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
                      <div>
                        ...
                        <div className="flex flex-wrap gap-1.5">
                          {ALL_SIZES.map(size => {
                            ...
                            return (
                              <button key={size} onPointerDown={() => toggleSize(fabric.id, variant.id, size)}
                                      className={`h-9 min-w-[40px] px-2 rounded-lg text-sm font-black border transition-all active:scale-[0.95] select-none ${isActive ? 'text-white' : 'text-text-muted border-white/10 hover:border-white/20'}`}
                                      style={isActive ? { background: 'linear-gradient(135deg, #3B82F6, #2563EB)', border: '1px solid rgba(59,130,246,0.5)', boxShadow: '0 0 8px rgba(59,130,246,0.3)' } : { background: 'rgba(255,255,255,0.04)' }}>
                                {size}
                              </button>
                            );
                          })}
                      ...
                      {Object.keys(variant.sizes).length > 0 && (
                        <button onPointerDown={() => setProportionTool(fabric.id, variant.id, !variant.showProportionTool)}
                                className={`flex flex-col items-center justify-center p-2.5 rounded-xl border transition-all ${variant.showProportionTool ? 'border-violet-500/40 bg-violet-500/15 text-violet-400' : 'border-white/10 text-text-muted hover:border-white/20'}`}>
                          <Calculator size={18} />
                    ...
                    {variant.showProportionTool && Object.keys(variant.sizes).length > 0 && (
                      <div className="p-3 space-y-3" style={{ background: 'rgba(139,92,246,0.06)', borderBottom: '1px solid rgba(139,92,246,0.12)' }}>
                        <div className="flex items-center gap-2 text-violet-400 text-sm font-black"><Calculator size={15} /> Calculadora de Proporção</div>
                        <p className="text-xs text-text-secondary leading-relaxed">Digite os fatores para distribuir as peças entre os tamanhos (ex: P=1, M=2, G=1).</p>
                          ...
                          {ALL_SIZES.filter(s => variant.sizes[s]).map(size => (
                            <div key={size} className="flex flex-col gap-1 w-16">
                              ...
                              <input type="number" min="0" inputMode="numeric"
                                     ...
                                     className="w-full h-10 rounded-lg text-center font-black text-sm text-text-primary bg-white/5 border border-violet-500/20 focus:outline-none focus:ring-1 focus:ring-violet-500/40" placeholder="0" />
                            </div>
                          ))}
                        </div>
                        <div className="flex justify-end">
                          <button onPointerDown={() => applyProportions(fabric.id, variant.id)}
                                  className="flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-black text-white active:scale-[0.97]"
                    ...
                    {Object.keys(variant.sizes).length > 0 && (
                      <div className="p-3 grid grid-cols-3 gap-2">
                        {ALL_SIZES.filter(s => variant.sizes[s]).map(size => {
                          ...
                          return (
                            <button key={size}
                                    onPointerDown={() => openNumpad({ fabricId: fabric.id, variantId: variant.id, size, label: `${variant.color || 'Cor'} — ${size}` }, cellVal)}
                                    className={`rounded-xl border-2 p-3 flex flex-col items-center gap-0.5 transition-all active:scale-[0.96] ${isEditing ? 'border-blue-500/70' : cellVal ? 'border-white/15 hover:border-white/25' : 'border-dashed border-white/10 hover:border-white/20'}`}
                                    style={isEditing ? { background: 'rgba(59,130,246,0.15)', boxShadow: '0 0 12px rgba(59,130,246,0.3)' } : cellVal ? { background: 'rgba(255,255,255,0.05)' } : { background: 'rgba(255,255,255,0.02)' }}>
                              <span className={`text-[10px] font-black uppercase tracking-widest ${isEditing ? 'text-factory-cut' : 'text-text-muted'}`}>{size}</span>
                              <span className={`text-2xl font-mono font-black tabular-nums ${isEditing ? 'text-factory-cut' : cellVal ? 'text-text-primary' : 'text-text-muted/40'}`}>{cellVal || '0'}</span>
                    ...
                    {Object.keys(variant.sizes).length > 0 && (
                      <div className="flex items-center justify-between px-4 py-2.5" style={{ background: 'rgba(255,255,255,0.03)', borderTop: '1px solid rgba(255,255,255,0.06)' }}>
                        <span className="text-[10px] text-text-muted font-black uppercase tracking-widest">Total da cor</span>
                        <span className="font-mono font-black text-text-primary tabular-nums">{variantTotal.toLocaleString('pt-BR')} pçs</span>
                      </div>
                    )}
                  </div>
                );
              ...
              <button onPointerDown={() => addVariant(fabric.id)}
                      className="w-full border-2 border-dashed rounded-xl py-3.5 flex items-center justify-center gap-2 text-sm font-bold text-text-muted hover:text-text-secondary transition-colors active:scale-[0.98]"
        ...
        <button onPointerDown={addFabric}
                className="w-full rounded-xl py-4 flex items-center justify-center gap-2 text-sm font-black text-text-secondary active:scale-[0.98] transition-all"
        ...
        <section className="space-y-2">
          ...
          <textarea value={notes} onChange={e => setNotes(e.target.value)} rows={3}
                    placeholder="Instruções especiais, referências, anotações..." className={`${inputClass} resize-none text-sm`} />
        </section>
      ...
      {!activeCell && (
        <div className="fixed bottom-16 left-0 right-0 z-40 p-3"
             style={{ background: 'linear-gradient(to top, #060B18 80%, transparent)' }}>
          <button id="submit-edit-btn" onPointerDown={handleSubmit} disabled={submitting}
                  className="w-full max-w-2xl mx-auto flex h-14 rounded-xl font-black text-base text-white items-center justify-center gap-2 active:scale-[0.98] transition-all disabled:opacity-50 disabled:pointer-events-none"
                  style={{ background: 'linear-gradient(135deg, #10B981, #059669)', boxShadow: '0 0 24px rgba(16,185,129,0.4)' }}>
            {submitting ? 'Salvando Alterações...' : <><Check size={18} /> Salvar Alterações</>}
          </button>
        </div>
      )}
      ...
      {activeCell && (
        <>
          ...
          <div className="fixed bottom-0 left-0 right-0 z-50 shadow-2xl">
            <Numpad value={numpadValue} onChange={handleNumpadChange} onEnter={closeNumpad} onClose={closeNumpad} label={activeCell.label} />
          </div>
        </>
      )}
    </>
