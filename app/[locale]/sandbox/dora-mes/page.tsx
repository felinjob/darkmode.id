...
import { subscribeToOrders, subscribeToDeletedOrders, seedInitialDataIfEmpty, restoreOrder, permanentDeleteOrder, softDeleteOrder, updateOrderStatus } from '@/services/dora/op-service';
...
const STATUS_CONFIG: Record<StageStatus, { label: string; color: string; bg: string; glow: string; borderColor: string; stepIndex: number }> = {
  FILA:          { label: 'FILA',       color: 'text-factory-alert', bg: 'bg-amber-500/15',   glow: 'rgba(245,158,11,0.25)',  borderColor: '#F59E0B', stepIndex: 0 },
  ENFESTO_CORTE: { label: 'CORTE',      color: 'text-factory-cut',   bg: 'bg-blue-500/15',    glow: 'rgba(59,130,246,0.25)',  borderColor: '#3B82F6', stepIndex: 1 },
  COSTURA:       { label: 'COSTURA',    color: 'text-factory-sew',   bg: 'bg-teal-500/15',    glow: 'rgba(20,184,166,0.25)',  borderColor: '#14B8A6', stepIndex: 2 },
  REVISAO_QC:    { label: 'REVISÃO',    color: 'text-factory-qc',    bg: 'bg-emerald-500/15', glow: 'rgba(16,185,129,0.25)',  borderColor: '#10B981', stepIndex: 3 },
  FINALIZADO:    { label: 'FINALIZADO', color: 'text-factory-done',  bg: 'bg-violet-500/15',  glow: 'rgba(139,92,246,0.15)',  borderColor: '#8B5CF6', stepIndex: 4 },
};
...
function OrderCard({ order, onClick, onDelete, onLiberar }: OrderCardProps) {
  ...
  if (order.timestamps.deadline && order.status !== 'FINALIZADO') {
    ...
    if (d.getTime() < Date.now()) isLate = true;
  }
  ...
  return (
    ...
    >
      {/* Dynamic left border highlight based on status */}
      <div
      ...
      {isUrgent && (
        <div className="absolute top-0 left-0 right-0 h-[2px] animate-pulse-glow"
      ...
      <div className="p-4 pl-5">
        <div className="flex items-start justify-between gap-3 mb-4">
          <div className="min-w-0">
            <div className="flex items-center gap-2 mb-1">
              <p className="font-mono font-black text-xl text-text-primary tracking-tight leading-none group-hover:text-blue-400 transition-colors">
                OP-{order.opNumber}
              </p>
              {isLate && (
                <span className="text-[9px] font-black uppercase bg-red-500/20 text-factory-scrap px-1.5 py-0.5 rounded border border-red-500/30">
                  Atrasada
                </span>
              )}
            </div>
            <p className="text-text-secondary font-semibold text-sm truncate">{order.clientName}</p>
          </div>
          ...
          <div className="flex flex-col items-end gap-2 shrink-0">
            ...
            {isUrgent && (
              <span className="flex items-center gap-1 text-[10px] font-black text-factory-scrap animate-pulse-glow">
        ...
        {/* Visual Progress Bar (Kanban style embedded) */}
        <div className="mb-4">
          <div className="flex items-center justify-between text-[9px] font-black uppercase tracking-widest text-text-muted mb-1.5 px-1">
            <span className={cfg.stepIndex >= 0 ? 'text-factory-alert' : ''}>Fila</span>
            <span className={cfg.stepIndex >= 1 ? 'text-factory-cut' : ''}>Corte</span>
            <span className={cfg.stepIndex >= 2 ? 'text-factory-sew' : ''}>Costura</span>
            <span className={cfg.stepIndex >= 3 ? 'text-factory-qc' : ''}>QC</span>
            <span className={cfg.stepIndex >= 4 ? 'text-factory-done' : ''}>Pronto</span>
          </div>
          <div className="w-full h-1.5 rounded-full flex gap-1 bg-transparent">
            {[0, 1, 2, 3, 4].map((step) => {
              ...
              if (isPast || isCurrent) {
                if (step === 0) stepBg = '#F59E0B'; // Fila
                if (step === 1) stepBg = '#3B82F6'; // Corte
                if (step === 2) stepBg = '#14B8A6'; // Costura
                if (step === 3) stepBg = '#10B981'; // QC
                if (step === 4) stepBg = '#8B5CF6'; // Pronto
              }
          ...
          <div>
            ...
            <div className="flex items-center gap-1.5 text-text-primary">
              ...
              <span className="font-mono font-black text-sm">
                {order.totalEstimatedPieces.toLocaleString('pt-BR')} <span className="text-[10px] text-text-muted uppercase">pçs</span>
              </span>
            </div>
          </div>
            ...
            {onDelete && (
              <button
                ...
                className="w-9 h-9 rounded-xl bg-white/5 hover:bg-red-500/15 border border-white/5 hover:border-red-500/30
                           flex items-center justify-center text-text-muted hover:text-factory-scrap transition-all"
...
export default function Home() {
  ...
  useEffect(() => {
    let loaded1 = false; let loaded2 = false;
    const unsub1 = subscribeToOrders(fetched => { setOrders(fetched); loaded1 = true; if (loaded1 && loaded2) setLoading(false); });
    const unsub2 = subscribeToDeletedOrders(fetched => { setDeletedOrders(fetched); loaded2 = true; if (loaded1 && loaded2) setLoading(false); });
    return () => { unsub1(); unsub2(); };
  }, []);

  const handleSeedData = async () => {
    setIsSeeding(true);
    try { await seedInitialDataIfEmpty(); } catch { } finally { setIsSeeding(false); }
  };
  ...
  const stats = useMemo(() => {
    ...
    activeOrders.forEach(o => {
      ...
      if (o.timestamps.deadline) {
        const d = (o.timestamps.deadline as any).seconds ? (o.timestamps.deadline as any).seconds * 1000 : (o.timestamps.deadline as any);
        if (d < now) late++;
      }
  ...
  const handleDelete = async (e: React.MouseEvent, id: string) => {
    e.stopPropagation();
    if (window.confirm('Mover esta OP para a lixeira?')) { try { await softDeleteOrder(id); } catch { } }
  };

  const handleLiberar = async (e: React.MouseEvent, id: string) => {
    e.stopPropagation();
    try { await updateOrderStatus(id, 'ENFESTO_CORTE'); } catch { }
  };
  ...
  return (
    <main className="p-4 pb-24 space-y-5 max-w-2xl mx-auto">
      ...
      <header className="pt-2 pb-2">
        ...
        <p className="text-xs text-text-secondary mt-1 capitalize flex items-center gap-1.5">
          <Activity size={12} className="text-factory-qc" />
          {today}
        </p>
      </header>
      ...
      {!loading && stats.active > 0 && tab === 'ATIVAS' && (
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
          <div className="rounded-2xl p-3 flex flex-col justify-center"
            ...
            <p className="font-mono font-black text-xl text-text-primary">{stats.totalPieces.toLocaleString('pt-BR')} <span className="text-xs font-bold text-text-muted">pçs</span></p>
          </div>
          ...
          <div className="rounded-2xl p-3 flex flex-col justify-center"
            ...
            <p className={`text-[10px] font-black uppercase tracking-widest mb-1 ${stats.urgent > 0 ? 'text-factory-scrap' : 'text-text-muted'}`}>Urgentes</p>
            <div className="flex items-center gap-2">
              <p className="font-mono font-black text-xl text-text-primary">{stats.urgent}</p>
              {stats.urgent > 0 && <Zap size={14} className="text-factory-scrap animate-pulse" />}
            </div>
          </div>
          ...
          <div className="rounded-2xl p-3 flex flex-col justify-center col-span-2 sm:col-span-1"
            ...
            <p className={`text-[10px] font-black uppercase tracking-widest mb-1 ${stats.late > 0 ? 'text-factory-alert' : 'text-text-muted'}`}>Atrasos</p>
            <p className="font-mono font-black text-xl text-text-primary">{stats.late} <span className="text-xs font-bold text-text-muted">OPs</span></p>
          </div>
        </div>
      )}
        ...
        {[
          ...
          <button
            ...
            className={`flex-1 py-2.5 rounded-lg text-[10px] font-black uppercase tracking-widest transition-all duration-200 ${
              tab === t.id ? 'text-text-primary shadow-lg' : 'text-text-muted hover:text-text-secondary hover:bg-white/5'
            }`}
            style={tab === t.id ? {
      ...
      {loading ? (
        <div className="space-y-4 pt-2">
          {[1, 2, 3].map(i => <div key={i} className="rounded-2xl h-44 skeleton" />)}
        </div>
      ) : tab === 'LIXEIRA' ? (
        deletedOrders.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-24 gap-4 text-center">
            ...
            <p className="text-text-secondary font-black text-sm uppercase tracking-widest">Lixeira vazia</p>
          </div>
        ) : (
          <div className="space-y-3">
            {deletedOrders.map(order => {
              const deletedDate = order.deletedAt?.toDate ? order.deletedAt.toDate() : new Date();
              ...
              return (
                <div key={order.id} className="rounded-2xl p-4 opacity-75 hover:opacity-100 transition-opacity"
                  ...
                  <div className="flex justify-between items-start mb-2">
                    <p className="font-mono font-black text-lg text-text-secondary line-through">OP-{order.opNumber}</p>
                    <span className="flex items-center gap-1 text-[10px] font-black uppercase tracking-widest text-factory-scrap bg-red-500/10 border border-red-500/20 px-2 py-1 rounded-full">
                      <Clock size={11} /> {Math.max(0, daysLeft)}d restantes
                    </span>
                  </div>
                  <p className="text-text-secondary text-sm font-semibold mb-4">{order.clientName}</p>
                  <div className="flex gap-2">
                    <button onClick={() => restoreOrder(order.id)}
                            className="flex-1 py-2.5 rounded-xl text-xs font-black uppercase tracking-widest text-factory-cut bg-blue-500/15 hover:bg-blue-500/25 border border-blue-500/30 active:scale-[0.98] transition-all flex items-center justify-center gap-1.5">
                      <RotateCcw size={14} /> Restaurar
                    </button>
                    <button onClick={() => { if (confirm('Exclusão definitiva não pode ser desfeita. Confirmar?')) permanentDeleteOrder(order.id); }}
                            className="flex-1 py-2.5 rounded-xl text-xs font-black uppercase tracking-widest text-factory-scrap bg-red-500/15 hover:bg-red-500/25 border border-red-500/30 active:scale-[0.98] transition-all">
                      Excluir p/ sempre
                    </button>
                  </div>
                </div>
        ...
        <div className="rounded-3xl p-8 text-center space-y-5 mt-4 shadow-2xl"
          ...
          <div>
            <p className="text-text-primary font-black text-xl tracking-tight">Fábrica Livre</p>
            <p className="text-text-secondary text-sm mt-2 leading-relaxed">Nenhuma ordem de produção em andamento. Adicione novas OPs para movimentar a produção.</p>
          </div>
          <button onClick={handleSeedData} disabled={isSeeding}
            ...
            {isSeeding ? 'Carregando Dados...' : 'Gerar Pedidos de Teste'}
          </button>
        </div>
        ...
        <div className="space-y-4 animate-fade-in pt-1">
          {displayOrders.map(order => (
            <OrderCard
              key={order.id}
              order={order}
              onClick={() => router.push(`/pt/sandbox/dora-mes/ordens/${order.id}`)}
